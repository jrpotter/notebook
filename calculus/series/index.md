...
Let $\alpha, \beta \in \mathbb{C}$ and both $\sum a_n$ and $\sum b_n$ be convergent infinite series of [[fields#Complex|complex]] terms. Then the series $\sum\, (\alpha a_n + \beta b_n)$ also converges and its sum is given by equation $$\sum_{k=1}^{\infty}\, (\alpha a_n + \beta b_n) = \alpha \sum_{k=1}^{\infty} a_n + \beta \sum_{k=1}^{\infty} b_n.$$
...
Let $(a_n)_{n \geq 0}$ and $(b_n)_{n \geq 0}$ be two sequences of [[fields#Complex|complex]] numbers such that $$a_n = b_n - b_{n + 1} \quad\text{for}\quad n = 1, 2, 3, \ldots$$
