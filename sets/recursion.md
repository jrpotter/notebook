...
> Assume $<$ is a {strict well ordering} on set $A$. Assume {for all $f$} there exists a {unique $y$} such that {$\gamma(f, y)$}. Then there exists a {unique function $F$} with {domain $A$} such that, for {all $t \in A$}, {$\gamma(F \restriction \mathop{\text{seg}_<}t, F(t))$}.
