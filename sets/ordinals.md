...
Then $\alpha = \mathop{\text{ran}}(E)$ is called the **epsilon-image** (denoted $\in$-image) of the well-ordered [[relations#Structures|structure]] $\langle A, < \rangle$. The name "$\in$-image" derives from the fact that $\alpha$ is [[well#By Epsilon|well ordered by epsilon]].
