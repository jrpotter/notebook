...
This group is the minimum subgroup of $G$ that contains $a$. An element $a$ of a group $G$ **generates** $G$ if $\langle a \rangle = G$. Alternatively, we call $a$ a **generator for** $G$. A group $G$ is **cyclic** if there is some element $a \in G$ that generates $G$.
